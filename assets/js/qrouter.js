    ...
    Q = function (W, D, HTML, hash, view, arg, _arg, i, index, Regex, key, Q) {
        ...
        onhashchange = function () {
            ...
            if (hash.lastIndexOf("?") > 0) {
                hash = hash.substring(hash.lastIndexOf('?'), 0);
            }

            //去除数字参数
            if (/[0-9]+/.test(hash.substring(hash.lastIndexOf('/') + 1, hash.length))) {
                hash = hash.substring(hash.lastIndexOf('/'), 0);
            }
        ...
        Q = {
            ...
            V: function () {
                //console.log('Q.js <https://github.com/itorr/q.js> create in 2014/12/28');
                //console.log('qrouter.js <https://github.com/Rhyheart/rhyheart.html.qrouter.git> modify in 2018/11/13');
                return this
            },
            go: function (u) {
                location.hash = '#' + key + u;
